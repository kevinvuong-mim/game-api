...
## 1. ensureNextYearPartition

**Schedule:** Ngày 1 mỗi tháng lúc 3:00 sáng (`0 3 1 * *`)

**Purpose:**

- Tự động tạo partition `game_results_<YYYY>` cho **năm tiếp theo** nếu chưa tồn tại.
- Tránh lỗi insert khi dữ liệu `createdAt` rơi vào năm mới mà chưa có partition tương ứng.
- Giữ bảng `game_results` (partition theo `createdAt`) hoạt động ổn định theo thời gian mà không cần can thiệp thủ công.

**Operation Logic:**

1. Tính `nextYear = năm hiện tại + 1` và `tableName = game_results_<nextYear>`.
2. Kiểm tra partition đã tồn tại chưa bằng query `pg_class` (`SELECT EXISTS ... WHERE relname = tableName`).
3. Nếu partition **đã tồn tại** → ghi log và skip (idempotent).
4. Nếu partition **chưa tồn tại** → tạo bằng raw SQL:
   - `CREATE TABLE game_results_<nextYear> PARTITION OF game_results`
   - `FOR VALUES FROM ('<nextYear>-01-01') TO ('<nextYear+1>-01-01')`
5. Ghi log khi tạo partition thành công.

**Startup trigger (bổ sung):**

- Ngoài cron, `MaintenanceService` cũng gọi `ensureNextYearPartition()` trong `onModuleInit()`.
- Đảm bảo partition cho năm tiếp theo tồn tại ngay khi app khởi động, không cần chờ đến ngày 1 tháng sau.
