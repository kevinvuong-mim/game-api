...
| Push payload        | FCM `data: { type, route }` — `type`: `top_100_entered`, `top_100_exited`, `saturday_rank`; `route`: `Leaderboard` | In-app navigation; local daily reward chỉ dùng `route: DailyReward` (không từ backend)                                                                                                               |
